...
export function parsePatientAccountStatusBody(body) {
  ...
  if (!['active', 'inactive', 'pending'].includes(status)) {
    throw badRequest('status must be active, inactive, or pending.');
  }
...
export async function registerPatientAccount(input) {
  ...
  try {
    ...
    const patient = await findMatchingPatient(connection, input);
    ...
    const [rows] = await connection.execute(
      `SELECT AccountId, PatNum, Email, FirstName, LastName, Phone, Birthdate, DriverLicense, Status, CreatedAt, LastLoginAt
       FROM luk_patient_accounts
       WHERE AccountId = ?
       LIMIT 1`,
      [result.insertId]
...
export async function loginPatientAccount(input) {
  ...
  try {
    const [rows] = await connection.execute(
      `SELECT AccountId, PatNum, Email, PasswordHash, FirstName, LastName, Phone, Birthdate, DriverLicense, Status, CreatedAt, LastLoginAt
       FROM luk_patient_accounts
       WHERE Email = ?
       LIMIT 1`,
      [input.email]
...
export async function listPatientAccounts(query = {}) {
  ...
  const [rows] = await pool.execute(
    `SELECT AccountId, PatNum, Email, FirstName, LastName, Phone, Birthdate, DriverLicense, Status, CreatedAt, UpdatedAt, LastLoginAt
     FROM luk_patient_accounts
     ${where}
