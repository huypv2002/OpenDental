...
set "TASK_PREFIX=LUK Dental SMS UIAutomation"
set "THIS_FILE=%~f0"
...
set "APP_EXE=%~dp0LUK Dental SMS Reminder Tool UIAutomation.exe"
set "START_REMINDER_FLAG=%~dp0scheduler-start-reminder.flag"
echo [%date% %time%] Start Monitoring task started. >> "%RUN_LOG%"
whoami >> "%RUN_LOG%" 2>&1
echo Working folder: %CD% >> "%RUN_LOG%"

taskkill /IM "LUK Dental SMS Reminder Tool UIAutomation.exe" /F >nul 2>nul
powershell -NoProfile -ExecutionPolicy Bypass -Command "Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -like '*sms-reminder-tool-uiautomation*sms_reminder_app.py*' } | ForEach-Object { Invoke-CimMethod -InputObject $_ -MethodName Terminate | Out-Null }" >nul 2>nul
