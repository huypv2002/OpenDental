# LUK Dental SMS Reminder Tool - UIAutomation Trial

Independent trial copy of the SMS reminder tool using
[Python-UIAutomation-for-Windows](https://github.com/yinkaisheng/Python-UIAutomation-for-Windows)
for Phone Link and Open Dental desktop control. The original `sms-reminder-tool`
folder remains on the `pywinauto` backend.
...
- Phone Link automation through `uiautomation` on Windows.
- UIAutomation COM initialization inside every Qt worker thread.
- A separate EXE and scheduler task prefix, so this build can be tested beside the original tool.
...
```bat
cd open-dental-bridge\sms-reminder-tool-uiautomation
python -m venv .venv
...
Phone Link does not provide a stable official SMS API. This trial uses the
`uiautomation` package and refreshes the real top-level UIAutomation control when
Phone Link rebuilds its XAML tree. Keep Phone Link logged in and visible while testing.
